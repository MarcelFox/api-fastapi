...
class MongoRepository(AbstractRepository[T]):
    def __init__(self, db_name: str, user: str, password:str, collection_name: str, host: str, port: str):
        super().__init__(db_name, user, password, host, port)
        self.client = MongoClient(f'mongodb+srv://{user}:{password}@{host}:{port}/{db_name}')
